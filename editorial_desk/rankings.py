...
class RankingsClient:
    ...
    def redraft_daddy_player_values(self) -> list[dict[str, Any]]:
        return self._player_values(
            REDRAFT_DADDY_PLAYER_VALUES_URL,
            label="Redraft Daddy",
        )

    def _player_values(self, url: str, *, label: str) -> list[dict[str, Any]]:
        ...
        if not isinstance(payload, list):
            raise ValueError(f"{label} player response was not a list")
        return payload
