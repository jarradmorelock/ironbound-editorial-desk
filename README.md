...
not reconstruct movement from its own weekly history.

The handoff supplies official Power Rankings and playoff forecast rows by roster
ID plus the exact Power Rankings PNG and Playoff Forecast PNG generated by that
Tuesday ranking run. The ranking engine explicitly distinguishes the upcoming
`ranking_week` shown on its graphic from `results_through_week`, the completed
week covered by the magazine. Editorial Desk validates the latter, so a Week 2
results magazine can correctly consume the Tuesday Week 3 ranking board.

Editorial Desk verifies each image checksum, copies the assets into the flagship
publication package, attaches them to the research email, and tells the
downstream publisher to place them unchanged rather than redrawing them. Once a
ranking handoff advertises the asset contract, a missing ranking graphic keeps
flagship research incomplete. Pre-asset legacy handoffs remain readable for
